				...
				<form action="<?= base_url('admin/Rute_Controller/store'); ?>" method="post">
					<div class="modal-body">
						...
						<div class="mb-3">
							...
							<select name="id_transportasi" id="trasnport" class="form-control">
								<?php foreach ($transportasi as $tp):  ?>
									<option value="<?= $tp->id_type_transportasi ?>"><?= $tp->nama_type ?></option>
								<?php endforeach; ?>
							</select>
						</div>
					</div>
					...
					<form action="<?= base_url('admin/Rute_Controller/update'); ?>" method="post">
						...
						<div class="modal-body">
							...
							<div class="mb-3">
								...
								<select name="id_transportasi" id="trasnport" class="form-control">
									<?php foreach ($transportasi as $tp):  ?>
										<option value="<?= $tp->id_type_transportasi ?>"
											<?= ($tp->id_type_transportasi == $r['id_transportasi']) ? 'selected' : '' ?>><?= $tp->nama_type ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
			...
			$(".btnHapus").click(function() {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						window.location.href = "<?= base_url('admin/Rute_Controller/delete/') ?>" + id;
					}
				});
			});
		});
